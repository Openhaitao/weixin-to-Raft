...
function assertCanonicalPaths(): void {
  ...
  assert.match(wrapper, /resolveStartArgs\(rest,\s*bot\.wechatDir,\s*bot\.promptFile,\s*bot\.memoryDir\)/);
  assert.match(wrapper, /const memoryDir = path\.join\(agentHome,\s*["']memory["']\)/);
