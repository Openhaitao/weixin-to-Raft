...
import {
  ...
  resolveTouziyunAuthScript,
  resolveTouziyunBpScript,
  resolveTouziyunCreateScript,
  resolveTouziyunOptionsScript,
...
async function buildExtractionPrompt(originalText: string, request: ChatRequest): Promise<string> {
  ...
  return [
    ...
    "协议格式是严格的：每个 `::field:: 字段名` 必须独占一行，字段值从下一行开始。",
    "禁止把值写在 field header 同一行；不要写 `::field:: 项目名称 = xxx`，也不要写 `::field:: 项目名称：xxx`。",
    "正确示例：",
    "::field:: 项目名称",
    "星海科技",
    "::field:: 公司创始人",
...
async function runTouziyunSubmit(args: { draftId: string; fields: Record<string, string> }): Promise<Record<string, unknown>> {
  ...
  return runTouziyunScriptJson(submitArgs, { timeoutMs: 180_000, stdin: JSON.stringify(draft) });
}
...
async function runTouziyunBpUpload(args: { projectId: string; filePath: string; fileName: string }): Promise<Record<string, unknown>> {
  ...
  return runTouziyunScriptJson(uploadArgs, { timeoutMs: 600_000 });
}

export class WechatProjectCreateFlow {
  private readonly states = new Map<string, FlowState>();
  private readonly stateFile = resolveWechatStateFile("project-create-text-flow.json");
  ...
  private async submitDraft(conversationId: string, state: Extract<FlowState, { phase: "draft" }>): Promise<ChatResponse> {
    ...
    if (result.needAuth) {
      const authResponse = await startTouziyunTextAuth("我会继续提交这个项目草稿。");
